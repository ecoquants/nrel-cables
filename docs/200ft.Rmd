...
ter_geo         <- here("data/200ft/territories.geojson")
lns_geo         <- here("data/lns.geojson")
cables_geo      <- here("data/200ft/cables.geojson")
depth_0_tif     <- here("../nrel-uses/data/layers/depth/_depth_epsg4326.tif")
depth_tif       <- here("data/200ft/depth.tif")
cables_b_tif    <- here("data/200ft/cables_b.tif")
depth_m_tif     <- here("data/200ft/depth_m.tif")
depth_mr_tif    <- here("data/200ft/depth_mr.tif")
depth_mr_geo    <- here("data/200ft/depth_mr.geojson")
cables_td_geo   <- here("data/200ft/cables_lns.geojson")
ply_cables_geo  <- here("data/200ft/cables_buf.geojson")
tide_max_tif    <- here("data/200ft/tide_max.tif")
tide_tif        <- here("data/200ft/tide.tif")
tide_geo        <- here("data/200ft/tide.geojson")
cables_tide_geo <- here("data/200ft/cables_buf200ft_tide.geojson")
wave_0_geo      <- here("data/wave.geojson")
wave_geo        <- here("data/200ft/wave.geojson")
wind_0_geo      <- here("data/wind.geojson")
wind_geo        <- here("data/200ft/wind.geojson")
cables_tide_geo <- here("data/200ft/cables_buf_tide.geojson")
cables_wave_geo <- here("data/200ft/cables_buf_wave.geojson")
cables_wind_geo <- here("data/200ft/cables_buf_wind.geojson")
...
geo_to_csv <- function(geo){
  # geo = geos[1]
  path_in  <- glue("{dir_out}/{geo}")
  path_out <- glue("{dir_out}/tables/{path_in %>% basename() %>% fs::path_ext_remove()}.csv")
...
if (!file.exists(depth_tif)){
  r_depth <- raster(depth_wrld_tif) %>% # x: [0,360]
    crop(cables_sf %>% as("Spatial")) # x: [141,296]
...
if (!file.exists(cables_td_geo)){
  ...
  #st_drop_geometry(lns_cables_td)
  #file.remove(cables_td_geo)
  write_sf(lns_cables_td, cables_td_geo)
}
lns_cables_td <- read_sf(cables_td_geo)

# buffer cables out 200 ft ----
if (!file.exists(cables_td_geo)){
  ply_cables <- lns_cables_td %>% 
    mutate(
      geometry = map(geometry, buffer_gcs, set_units(200, ft)) %>% 
        unlist(recursive = F) %>% 
        st_sfc(crs=4326),
      area_km2 = st_area(geometry) %>% set_units(km^2))
  #st_drop_geometry(ply_cables)
  #file.remove(ply_cables_geo)
...
if (!file.exists(tide_geo)){
  ply_tide <- rasterToPolygons(r_tide, dissolve=T) %>% 
    ...
    mutate(
      area_km2 = st_area(geometry) %>% set_units(km^2))
  # st_drop_geometry(ply_tide) %>% View()
...
# prep wave
if (!file.exists(wave_0_geo)){
  ply_wave <- read_sf() %>% # x:[171,297]
...
#st_drop_geometry(ply_wave) %>% View()

# prep wind
if (!file.exists(wind_0_geo)){
  ply_wind <- read_sf(here("data/wind.geojson")) %>% # x:[199,294]
    mutate(
      region = case_when(
        region %in% c(
...
#st_drop_geometry(ply_cables_tide)
...
#st_drop_geometry(ply_cables_wave)
...
#st_drop_geometry(ply_cables_wind)

# geojson to csv ----
dir_out <- here("data/200ft")
geos <- list.files(dir_out, ".*\\.geojson") %>% 
  setdiff(c("cables.geojson", "depth_mr.geojson"))
walk(geos, geo_to_csv)
```
...
dir_tbl <- here("data/200ft/tables")

tbl_es1 <- read_csv(file.path(dir_tbl, "regions.csv")) %>% 
  left_join(
    read_csv(file.path(dir_tbl, "cables.csv")) %>% 
      group_by(region) %>% 
      summarize(
...
write_csv(tbl_es1, file.path(dir_tbl, "table_ES-1.csv"))
...
Source: [table_ES-1.csv](https://github.com/ecoquants/nrel-cables/blob/master/data/200ft/tables/table_ES-1.csv)
...
tbl_es2 <- read_csv(file.path(dir_tbl, "energy-depth_3z.csv")) %>% 
  group_by(energy=form) %>% 
  summarize(viable_km2 = sum(area_km2)) %>% 
  left_join(
    bind_rows(
      read_csv(file.path(dir_tbl, "cables_tide.csv")) %>% 
        filter(depth_beg == 0, tide_beg >= 500) %>% 
        summarize(cable_km2 = sum(area_km2)) %>%
        mutate(energy = "tide"),
      read_csv(file.path(dir_tbl, "cables_wave.csv")) %>% 
        filter(depth_beg <= 100, wave_beg >= 10) %>% 
        summarize(cable_km2 = sum(area_km2)) %>%
        mutate(energy = "wave"),
      read_csv(file.path(dir_tbl, "cables_wind.csv")) %>% 
...
#tbl_es2
write_csv(tbl_es2, file.path(dir_tbl, "table_ES-2.csv"))
...
Source: [table_ES-2.csv](https://github.com/ecoquants/nrel-cables/blob/master/data/200ft/tables/table_ES-2.csv)

## 1. Regions & Cable Lengths

```{r}
tbl_1 <- read_csv(file.path(dir_tbl, "regions.csv")) %>% 
  select(
...
write_csv(tbl_1, file.path(dir_tbl, "table_1.csv"))
...
Source: [table_1.csv](https://github.com/ecoquants/nrel-cables/blob/master/data/200ft/tables/table_1.csv)

## 2. Regions & Cable Overlap Area

```{r}
tbl_2 <- read_csv(file.path(dir_tbl, "regions.csv")) %>% 
  select(
    region, region_km2=area_km2) %>% 
  left_join(
    read_csv(file.path(dir_tbl, "cables.csv")) %>% 
...
write_csv(tbl_2, file.path(dir_tbl, "table_2.csv"))
...
Source: [table_2.csv](https://github.com/ecoquants/nrel-cables/blob/master/data/200ft/tables/table_2.csv)

## 3. Binned Resource & Cable Overlap Area

```{r}
tbl_3 <- read_csv(file.path(dir_tbl, "energy-depth_3z.csv")) %>% 
  ...
  select(
    ...
    cable_km2, cable_pct_viable)
#tbl_3
write_csv(tbl_3, file.path(dir_tbl, "table_3.csv"))
...
Source: [table_3.csv](https://github.com/ecoquants/nrel-cables/blob/master/data/200ft/tables/table_3.csv)

## 4. Tidal Resource & Cable Overlap Area by Region

```{r}
tbl_4 <- read_csv(here("data/tide_depth_cables.csv")) %>% 
  filter(
    territory != "ALL",
    energy_factor != "0-500",
  ...
  select(
    ...
    cable_km2, cable_pct_viable)
#tbl_4
write_csv(tbl_4, file.path(dir_tbl, "table_4.csv"))
...
Source: [table_4.csv](https://github.com/ecoquants/nrel-cables/blob/master/data/200ft/tables/table_4.csv)

## 5. Wave Resource & Cable Overlap Area by Region

```{r}
tbl_5 <- read_csv(here("data/wave_depth_cables.csv")) %>% 
  filter(
    territory != "ALL",
    energy_factor != "0-10",
  ...
  select(
    ...
    cable_km2, cable_pct_viable)
#tbl_5
write_csv(tbl_5, file.path(dir_tbl, "table_5.csv"))
...
Source: [table_5.csv](https://github.com/ecoquants/nrel-cables/blob/master/data/200ft/tables/table_5.csv)

## 6. Wind Resource & Cable Overlap Area by Region

```{r}
tbl_6 <- read_csv(here("data/wind_depth_cables.csv")) %>% 
  filter(
    territory != "ALL",
    energy_factor != "<=7",
  ...
  select(
    ...
    cable_km2, cable_pct_viable)
#tbl_6
write_csv(tbl_6, file.path(dir_tbl, "table_6.csv"))
...
Source: [table_6.csv](https://github.com/ecoquants/nrel-cables/blob/master/data/200ft/tables/table_6.csv)
