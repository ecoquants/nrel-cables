...
```{r leaflet cables + buffer at depth classes}
library(rgeos)
library(geosphere)
library(sf)
library(tidyverse)
...
dcx2_rds = sprintf('../data/%s_2xdepth-classes.rds', region)
dcx3_rds = sprintf('../data/%s_3xdepth-classes.rds', region)
...
if(any(!file.exists(dcx2_rds), !file.exists(dcx3_rds))){
  ...
  data_frame(
    ...
    bind_rows(
      data_frame(
        depth_1x = seq(250 + d_incr/2, cellStats(depth_rc, 'max'), by=d_incr)) %>%
        mutate(
    ...
    mutate(
      depth_2x   = depth_1x*2,
      depth_3x   = depth_1x*3) %>% 
    write_csv(tbl_reclass_csv)
  
  # reclassify raster
  tbl_reclass = read_csv(tbl_reclass_csv)
  tbl_reclass %>% DT::datatable() # n=51
  
  r_d1x = reclassify(depth_rc, tbl_reclass %>% select(depth_from, depth_to, depth_1x)) # plot(r_d1x)
  system.time({
    p_d1x = rasterToPolygons(r_d1x, dissolve=T) # ca-coast: 5 min
  })
  ...
  system.time({
    lns_i = raster::intersect(lns_t, p_d1x) # morro-bay: 3 secs; ca-coast: 6.2 min
  })
  ...
  #for (d in bufs$depth){ # m = 8
  for (i in 1:nrow(bufs)){ # i = 1
    b = bufs$buf1x[i]
    cat(sprintf('%03d of %d: %d m\n', i, nrow(bufs), b))
  ...
  saveRDS(ply_dx2, dcx2_rds)
  saveRDS(ply_dx3, dcx3_rds)
}

ply_dx2 = readRDS(dcx2_rds)
ply_dx3 = readRDS(dcx3_rds)
