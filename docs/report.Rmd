...
map_cable = function(
  ...
  redo,
  fig){
  # ter = 'Hawaii'; fig = sprintf('figs/mapCable_%s.pdf', str_replace_all(ter,' ','-')); energy_sf = wind_sf; legend_title = sprintf('Submarine cables within %s EEZ.', ter); redo = T
...
fig = 'figs/mapCableTerritories.pdf' # redo=T
if (!file.exists(fig) | redo){
...
fig = 'figs/mapCableBuffers.pdf' # redo=T
if (!file.exists(fig) | redo){
...
```{r mapCable, fig.cap='Maps of tidal energy.', eval=F}
for (ter in ){
    # ter = 'Hawaii'
  fig = sprintf('figs/mapCable_%s.pdf', str_replace_all(ter,' ','-'))
  map_cable(
    ter = ter,
    fig = fig,
    redo = F)
...
```{r mapTide, fig.cap='Maps of tidal energy.', eval=F}
...
```{r mapWave, fig.cap='Maps of wave energy.', eval=F}
...
```{r mapWind, fig.cap='Maps of wind energy.', eval=F}
...
```{r srcTerritoryTemplate, include=F, eval=T}
src <- lapply(sort(unique(lns_d1x$territory)), function(ter) knit_expand(file='territory_template.Rmd'))
```
