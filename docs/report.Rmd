...
if (out_html){
  ...
  usa_tbl %>%
    kable(caption = 'Territories within the United States exclusive economic zone (EEZ).') # , booktabs=T)
}
```
...
# TODO: apply formatting for easy table viz -- split up columns
...
#   rownames = F,
#   options = list(
#     pageLength = nrow(tbl), dom='t', ordering=F, # all rows, off: n_entries/search/paging/ordering
...
wave_cbls = read_csv(wave_cbls_csv) %>%
  rename(`Wave energy (kW/m)`=energy_lbl) %>%
  mutate(
   cable2_pct     = cable2_pct,
   cable3_pct     = cable3_pct)
...
plot_wave = function(ter){ # ter = setdiff(unique(d_p$territory), 'ALL') 
  # ter = unique(d_p$territory) # ter = 'ALL'
  txt_adj = wave_cbls %>% 
    filter(territory %in% ter) %>% 
    summarize(txt_adj = max(area_km2)/1000*0.025) %>% 
    .$txt_adj
  
  p = ggplot(
      data=d_p %>% filter(territory %in% ter), 
      #data=d_p, 
      aes(x = energy, y = km2/1000, fill=type)) +
    geom_col() +
    geom_text(
      data=wave_cbls %>% filter(territory %in% ter), 
      #data=wave_cbls, 
      aes(label=pct_lbl, x=energy, y=area_km2/1000, fill=NULL), 
      position = position_nudge(y = txt_adj), size=2) +
    labs(
      #title=sprintf('%s: Wave Energy by Area and Power Class', ter), 
      title=sprintf('Wave Energy by Area and Power Class per US Territory'), 
      subtitle='with Cable Overlay (Minimum - Recommended %)', 
      x='Wave energy (kW/m)', y='Area (1,000 km^2)', fill='Type') +
    theme(
