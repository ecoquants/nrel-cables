...
```{r map_study-area, fig.cap='Map of NOAA Charted Submarine cables in the United States as of December 2012.'}
...
leaflet() %>% 
  ...
  addPolygons(
    data = dx2, group='buffer: 3*depth',
    color='red', stroke=F, fillOpacity = 0.5) %>%
  addPolygons(
    data = dx3, group='buffer: 2*depth', 
    color='red', stroke=F, fillOpacity = 0.5) %>%
  addPolylines(
    data = lns_d1x, group='cables', 
    color='black', opacity = 1, weight=1,
    popup = ~sprintf(
      "scaleBand: %s<br>description: %s<br>effectiveDate: %s<br>EEZ: %s<br>EEZ part: %d<br>Buffer (1x): %d",
      scaleBand, description, effectiveDate, GeoName, part, buf1x)) %>%
  addLayersControl(
...
wind_cbls %>%
  ...
  formatRound(
    ...
    digits=2) %>% 
  formatPercentage(
    c('pct_region','cable2_pct','cable3_pct','cable3_pct_region'), 
    digits=1)
```
...
wave_cbls %>%
  ...
  formatRound(
    ...
    digits=2) %>% 
  formatPercentage(
    c('pct_all','cable2_pct','cable3_pct','cable3_pct_all'), 
    digits=1)
```
