...
if ("rmarkdown.pandoc.to" %in% names(opts_knit$get()) && opts_knit$get("rmarkdown.pandoc.to") == 'html'){
...
[MarineRegions.org](http://marineregions.org/downloads.php) > Exclusive Economic Zones Boundaries (EEZ), version: World EEZ v9 (2016-10-21, 123 MB)
...
# TODO: intersect lines and USA EEZ
# usa_x_lns_sf = usa_sf %>%
#   st_intersects(lns_sf)
# 
# usa_lns_sf = usa_sf %>%
#   filter(st_intersects(., lns_sf) %>% lengths() > 0)

# usa_parts_sf <- st_cast(usa_sf, "POLYGON") %>%
#   mutate(
#     part = row_number())
# usa_parts_sf %>%
#     write_sf(usa_geo)
# 
# usa_parts_sf %>% as_data_frame() %>% select(-geometry) %>% View()
#lns_sf = st_transform(lns, leaflet:::epsg4326)

#usa = usa_sf
usa = usa %>%
  filter(part %in% lns_usa$part)
  
#pal <- colorFactor('Spectral', lns_usa$part)
pal <- colorFactor(
...
# lns_usa = read_sf('../data/lns_usa.geojson')

# lns_sum = lns_usa %>%
#   mutate(
#     length = sapply(st_geometry(lns_usa_sf), st_length)) %>%
#   group_by(GeoName) %>%
#   summarize(
#     n = n(),
...
### Depth + Cables: Morro Bay example

Initially, let's try the depth-varying buffer on a small experimental area of Morro Bay, CA where a floating offshore wind farm is currently proposed amongst [California Activities | BOEM](https://www.boem.gov/California/). We'll scale up to national later.
...
### Depth-Varying Buffer: Morro Bay example
...
**Next steps**. The above R code works well for this small area, but will not scale well to the national level. I'll implement the intersection, buffering and dissolve operations in a [PostGIS](http://postgis.net/) spatial database, which has the advantage of much more efficient spatial indexing and operations.
...
## Depth-Varying Buffer: CA Example
...
regions = list(
  all        = c(xmin = bb@xmin, ymin = bb@ymin, xmax = bb@xmax, ymax = bb@ymax),
  `ca-coast` = c(xmin = -126, xmax = -116, ymin = 32, ymax = 42),
  pacific    = c(xmin = -126, xmax = -116, ymin = 32, ymax = 42))
region = 'ca-coast'

dx2_rds = sprintf('../data/%s_2xdepth-classes.rds', region)
dx3_rds = sprintf('../data/%s_3xdepth-classes.rds', region)
tbl_reclass_csv = sprintf('../data/%s_depth-reclass.csv', region)


# clip depth to US W Coast (eg bottom right: https://www.google.com/maps/place/32N+116W)
# # US west coast
v = regions[[region]]
depth_rc = crop(depth_r, extent(v['xmin'], v['xmax'], v['ymin'], v['ymax']))
depth_rc[depth_rc > 0] = NA
depth_rc = depth_rc * -1
#plot(depth_rc)
...
## Buffering over Entire Area

In order to apply the depth-varying buffer over the entirety of the cable layer without hitting memory issues, I divided the area into 32 tiles.

Generated by:
- [create_cable-buffer.R](https://github.com/ecoquants/nrel-cables/blob/master/docs/create_cable-buffer.R)

```{r tiled buffering at depth classes}
library(tidyverse)
library(sp)
library(rgdal)
library(leaflet)
...
# pal <- colorNumeric(
#   c("blue","purple"),  # RColorBrewer::display.brewer.all()
#   values(depth_rcm), na.color = "transparent")

#xmin = -121.29; xmax = -120.59; ymin = 35.04; ymax = 35.55
leaflet() %>% 
  addProviderTiles("Stamen.TonerLite", group = "B&W") %>%
  addProviderTiles('Esri.OceanBasemap', group = "Ocean") %>%  #
  #addRasterImage(depth_rcm, colors = pal, opacity = 0.8) %>%
  #addLegend(
  #  pal = pal, values = values(depth_rcm),
  #  title = "depth_rcm") %>%
  addPolygons(
    data = ply_dx3, group='buffer: 3*depth',
    color='red', stroke=F, fillOpacity = 0.5) %>%
...
# names(ply_ca)
# 
# summary(ply_ca['MEANPO'])
# hist(ply_ca@data$MEANPO)

spplot(ply_ca['MEANPO'])
```
