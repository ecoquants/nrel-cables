...
if (out_html){
  ...
  m_html = '../tmp/map_study-area.html'
  m_png  = '../tmp/map_study-area.png'
  saveWidget(m, m_html, selfcontained=F)
  webshot(m_html, file=m_png, cliprect="viewport")
  #knitr::include_graphics(m_png)
  ...
  #   geom_map(fill='gray80', color='white',map=fifty_states) +
  #   geom_map(fill='gray80', color='white',map=fifty_states) +
  ...
  # wrld = map_data('world')
  #
  # p = ggplot() + geom_polygon(data=wrld, aes(x=long, y=lat, group=group), color='white', fill='gray10')
  # p
  #
  #
...
wind_tbl_fmt = 
  ...
  mutate(
    `cable (km2)` = sprintf('%s - %s', cable2_km2, cable3_km2),
    `cable (%)`   = sprintf('%0.2f - %0.2f%%', cable2_pct, cable3_pct)) %>%
  select(region, `speed m/s`, `cable (km2)`, `cable (%)`, region_km2=area_km2)
  # rename(
  #   rgn = region,
  #   rgn_km2      = area_km2,
  #   pct_rgn      = pct_region,
