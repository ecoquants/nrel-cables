...
if (!file.exists(tide_tif)){
  ...
  # intersect with USA territories
  system.time({
    tp = raster::intersect(tp, usa_rgn) # > 1 hr
  })
  
  # TODO: rewrite tide_csv w/ territory
  tp %>%
    as_tibble() %>%
    select(lon=LONGIT, lat=LATITU, pwr_wm2=MEANPO) %>%
    mutate(
...
# tide: rasterize and extract by territory
rgn_r = list(); rgn_v = list(); rgn_c = list()
for (rgn in unique(tp@data$territory)){ # rgn='East'
  
  pts = subset(tp, territory==rgn)
  rgn_r[[rgn]] = rasterize(
    pts,
...
if (!file.exists(wave_geo)){
  ...
  wave = raster::aggregate(wave, by=c('territory','energy_lbl','energy_num'))
...
# wave: intersect with cables & dissolve on territory & energy -----
...
# wave: summarize by territory & energy with cables into table -----
...
if (!file.exists(wind_geo)){
  ...
  # dissolve on territory & energy
  wind = raster::aggregate(wind, by=c('territory','energy_lbl','energy_num'))
...
# wind: intersect with cables & dissolve on territory & energy -----
if (any(!file.exists(wind_cbl2_geo), !file.exists(wind_cbl3_geo))){
  wind_cbl2 = raster::intersect(wind, cbl2) %>%
    raster::aggregate(by=c('territory','energy_lbl','energy_num'))
...
# wind: summarize by territory & energy with cables into table -----
if (!file.exists(wind_cbls_csv)){
  ...
  wind_cbls = wind@data %>%
    ...
    group_by(territory) %>%
    mutate(
      pct_territory        = area_km2 / sum(area_km2) * 100,
      cable2_pct_territory = cable2_km2 / sum(area_km2) * 100,
      cable3_pct_territory = cable3_km2 / sum(area_km2) * 100) %>%
    ungroup() %>%
    select(
      territory, energy_num, energy_lbl, area_km2, pct_territory,
      cable2_km2, cable2_pct, cable2_pct_territory, cable3_km2, cable3_pct, cable3_pct_territory) %>%
    arrange(territory, energy_num)
